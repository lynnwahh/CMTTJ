...
from cmttj.helper_file import (check_logfile, collate_results_csv_to_xlsx, create_results_folder, elapsed_time,
...
from cmttj.track_eval import annotate_video, evaluate_tracks, select_tracks, track_bacteria

__all__ = ['analyse', 'cmttj']
        ...
        if plots_eval:
            ...
            if settings['save video'] and '.csv' not in path:
                ...
                logger.warning(
                    ...
                    'when cmttj() is given a video as an argument. Optionally use annotate_video() from '
                    'cmttj.track_eval directly.')
...
def cmttj(paths=None, settings=None, result_folder=None, multiprocess=False):
    """
    Starts asynchronous multiprocessing of provided video file(s) with analyse().
    Due to the use of multiprocessing, cmttj() should be called in a
