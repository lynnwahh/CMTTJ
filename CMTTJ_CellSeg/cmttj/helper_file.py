...
def collate_results_csv_to_xlsx(path=None, save_path=None, csv_extension='statistics.csv'):
    ...
    :return: None
    """
    logger = logging.getLogger('cmttj').getChild(__name__)
    try:
        import xlsxwriter
    except ImportError:
...
def create_results_folder(path):
    ...
    """
    logger = logging.getLogger('cmttj').getChild(__name__)
    dir_form = '{}_Results/'.format(str(strftime('%y%m%d%H%M', localtime())))
...
def get_configs(tracking_ini_filepath=None):
    ...
    if isinstance(tracking_ini_filepath, collections.Mapping):  # lazy check for already generated settings
        ...
        try:
            ...
            # 2. [basic recording settings] rod shaped bacteria
            ...
            # 3. [advanced video settings] color filter
            ...
            # 4. [results settings] split results by
            # 5. split violin plots on
            ...
            # 6. [gaussian-sum fir filter settings] maximum horizon size
            ...
            settings_dict = {
                ...
                'exclude measurement when above x times average area': val_to_float_or_false(
                    adv_track.get('exclude measurement when above x times average area')),
            ...
            # Assertion checks, 不正确则触发 assertion error
...
def get_loggers(log_level=logging.DEBUG, logfile_name='./logfile.log', short_stream_output=False,
    ...
    logger = logging.getLogger('cmttj')
...
def logging_configurer(settings):
    ...
    :param settings: cmttj settings
    :type settings: dict
    :return: None
    """
    root = logging.getLogger('cmttj')
...
def stop_logging_queue(logger=None, settings=None):
    ...
    logger_list = [logger, logging.getLogger('cmttj').getChild(__name__), logging.getLogger('cmttj')]
