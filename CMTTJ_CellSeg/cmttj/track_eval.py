...
from cmttj.helper_file import (argrelextrema_groupby, create_results_folder, different_tracks, get_configs, get_data,
                               get_loggers, reshape_result, save_df_to_csv, save_list, sort_list)
from cmttj.plot_functions import angle_distribution_plot, large_xy_plot, rose_graph, violin_plot
from cmttj.tracker import CentroidTracker
...
def track_bacteria(video_path, settings=None, result_folder=None):
    ...
    while True:  # Loop over video
        ...
        if not ret and (frame_count == curr_frame_count + 1 or  # some file formats skip one frame
            ...
            logger.critical('Error during cap.read() with file {} in frame {}'.format(video_path, curr_frame_count))
        ...
        if settings['display video analysis']:  # Display current FPS on frame
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):  # Interrupt display on 'q'-keypress
                ...
                break
        cv2.imwrite('../inprocessing/{}_ori.png'.format(timer), ori_frame)
        cv2.imwrite('../inprocessing/{}.png'.format(timer), frame)
...
def evaluate_tracks(path_to_file, results_directory, df=None, settings=None, fps=None, **_):
    ...
    if settings['save large plots'] or settings['save rose plot']:
        ...
        if settings['save large plots']:
            save_df_to_csv(
                df=df,  # .reindex(columns=df_stats_columns),
                save_path=save_path.format('fortracks', '.csv')
            )
            large_xy_plot(df=df,
                          plot_title_name=plot_title_name,
